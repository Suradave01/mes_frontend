...
import ModalViewRole from "../../../components/user-management/roles/ModalViewRole";
...
const Index = () => {
  ...
  const columnDefs = [
    ...
    { headerName: "Description", field: "role_description" },
    {
      headerName: "State",
      field: "state",
      ...
      cellRendererFramework: (params: any) => (
        <div>
          <Space>
            ...
            <Link href={`/user-management/roles/edit/${params.data.id}`}>
              <Button>Edit</Button>
            </Link>
  ...
  const onDelete = async (id: number) => {
    if (window.confirm("Are you sure?")) {
      await roleService.delete(id).then(async (data) => {
        onGridReady(data);
      });
    }
  };
  ...
  const onUpdateStateActive = async (id: number) => {
    await roleService.active(id, null).then(async (data) => {
      onGridReady(data);
    });
  };
        ...
        <Row>
          ...
          >
            <div style={{ marginBottom: 10 }}>
              <Space>
                <Link href="/user-management/roles/add">
