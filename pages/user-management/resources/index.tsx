...
import ModalViewResource from "../../../components/user-management/resources/ModalViewResource";
...
const Index = () => {
  ...
  const columnDefs = [
    // { headerName: "#", field: "id" },
    { headerName: "#", field: "resource_code" },
    { headerName: "Resource Name", field: "resource_name" },
    { headerName: "Type", field: "resource_type" },
    {
      headerName: "State",
      field: "state",
      ...
      cellRendererFramework: (params: any) => (
        <div>
          <Space>
            ...
            <Link href={`/user-management/resources/edit/${params.data.id}`}>
              <Button>
                Edit
              </Button>
            </Link>
  ...
  const onDelete = async (id: number) => {
    if (window.confirm("Are you sure?")) {
      await resourceService.delete(id).then(async (data) => {
        onGridReady(data);
      });
    }
  };
  ...
  const onUpdateStateActive = async (id: number) => {
    await resourceService.active(id, null).then(async (data) => {
      onGridReady(data);
    });
  };
        ...
        <Row>
          ...
          >
            <div style={{ marginBottom: 10 }}>
              <Space>
                <Link href="/user-management/resources/add">
