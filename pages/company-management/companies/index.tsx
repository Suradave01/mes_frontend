...
import ModalViewCompany from "../../../components/company-management/companies/ModalViewCompany";
...
const Index = () => {
      ...
      cellRendererFramework: (params: any) => (
        <div>
          <Space>
            ...
            <Link href={`/company-management/companies/edit/${params.data.id}`}>
              <Button>Edit</Button>
            </Link>
  ...
  const onDelete = async (id: number) => {
    if (window.confirm("Are you sure?")) {
      await companyService.delete(id).then(async (data) => {
        onGridReady(data);
      });
    }
  };
  ...
  const onUpdateStateActive = async (id: number) => {
    await companyService.active(id, null).then(async (data) => {
      onGridReady(data);
    });
  };

  const onUpdateStateInactive = async (id: number) => {
    await companyService.inactive(id, null).then(async (data) => {
      onGridReady(data);
    });
  };
        ...
        <Row>
          ...
          >
            <div style={{ marginBottom: 10 }}>
              <Space>
                <Link href="/company-management/companies/add">
